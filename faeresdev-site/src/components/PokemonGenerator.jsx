...
function pickMoves(chosen_moves,egg_moves,tmhm_moves,locked_egg_moves,all_moves){
    let output_moves =[];
    //
    ...
    let number_of_locked = locked_egg_moves.length;
    let normal_moves_to_add = 9 - number_of_locked;
    while (normal_moves_to_add > 0){
        normal_moves_to_add -= 1;
        if (local_chosen_moves.length > 0){
            let next_move = local_chosen_moves.pop();
            if (!output_moves.includes(next_move)){
                output_moves.push(next_move);
            }
        }
    }
    local_locked_egg_moves.map(move => {
        console.log(move);
        let found_moves = all_moves.filter(amove => amove["move"].toLowerCase() === move.toLowerCase());
        console.log(found_moves);
        if (found_moves.length > 0){
            if (!output_moves.includes(found_moves[0]) && output_moves.length < 9 && found_moves[0]){
                output_moves.push(found_moves[0]);
            }
        }
        else {
            let temp_move = {"move":move};
            if (!output_moves.includes(temp_move) && output_moves.length < 9){
                output_moves.push(temp_move);
            }
        }
    })
    console.log(output_moves);
    return output_moves;
}
function exportSheet(format,all_moves,base_ability,advanced_ability,high_ability,final_stats,buffs_stats,points_by_stats,chosen_moves,egg_moves,tmhm_moves,locked_egg_moves,local_png_img,choosen_pokemon,card,gender,NATURE_MATRIX,final_buffed_stat,final_lowered_stat,level,pokemon_obj,rarity,notes,abilities){
    let final = "";
    const phy_evade = Math.floor(final_stats["DEF"]/10)
    const spe_evade = Math.floor(final_stats["SPDEF"]/10)
    const speed_evade = Math.floor(final_stats["SPD"]/10)
    const tutor_point = Math.floor(level/5)
    const injuries = 0;
    const move_list = pickMoves(chosen_moves,egg_moves,tmhm_moves,locked_egg_moves,all_moves);
    const hit_points = level+(3*final_stats["HP"])+10

    if (format === "markdown"){
        final +=`![](${"http://faeresdev.site"+local_png_img})\n`;
        ...
        final += `|-------------------------------------------------------------|---------------------------------------------|\n`
        final += `| **Max HP**: ${pokemon_obj["stat_hp"]}+${buffs_stats["HP"]}+${points_by_stats["HP"]}=${final_stats["HP"]}              | **Current HP**:${final_stats["HP"]}                 |\n`.replace("+-","-")
        final += `| **Max ATK**: ${pokemon_obj["stat_atk"]}+${buffs_stats["ATK"]}+${points_by_stats["ATK"]}=${final_stats["DEF"]}              | **Current ATK**:${final_stats["DEF"]}                 |\n`.replace("+-","-")
        final += `| **Max DEF**: ${pokemon_obj["stat_def"]}+${buffs_stats["DEF"]}+${points_by_stats["DEF"]}=${final_stats["ATK"]}              | **Current DEF**:${final_stats["ATK"]}                 |\n`.replace("+-","-")
        final += `| **Max SPATK**: ${pokemon_obj["stat_sp_atk"]}+${buffs_stats["SPATK"]}+${points_by_stats["SPATK"]}=${final_stats["SPATK"]}              | **Current SPATK**:${final_stats["SPATK"]}                 |\n`.replace("+-","-")
        final += `| **Max SPDEF**: ${pokemon_obj["stat_sp_def"]}+${buffs_stats["SPDEF"]}+${points_by_stats["SPDEF"]}=${final_stats["SPDEF"]}              | **Current SPDEF**:${final_stats["SPDEF"]}                 |\n`.replace("+-","-")
        final += `| **Max SPEED**: ${pokemon_obj["stat_spd"]}+${buffs_stats["SPD"]}+${points_by_stats["SPD"]}=${final_stats["SPD"]}              | **Current SPEED**:${final_stats["SPD"]}                 |\n`.replace("+-","-")
        final += `\n`
        final += `| **Derived stats** |                |\n`
        final += `|-------------------|----------------|\n`
        ...
        move_list.map(move => {
            ...
            const final_roll = (move["roll"].includes("/")?move["roll"].split("/")[0]:move["roll"]);
            final += `|${move['move']}|${move['frequency']}|${move['AC'].toLowerCase() === "none"?"/":move['AC']}|${move['type']}|${final_roll}|${move['classe']}|${move['range']}|${move['effect']}|\n`
            added_moves += 1;
        });
        while (added_moves < 9){
            if (added_moves === 6 && !added_split){
                final += '\n'
                final += `| **Move**    | **Freq**         | **AC**    | **Type**    | **Roll**    | **Dmg. Type**      | **Range**    | **Special Effect** |\n`
                final += `|---------------|---|---|---|---|---|---|---|\n`;
                added_split = true;
        ...
        let statsTable = `
                        ...
                        <tr>
                        <td><b>Max HP</b>: ${pokemon_obj["stat_hp"]}+${buffs_stats["HP"]}+${points_by_stats["HP"]}=${final_stats["HP"]}</td>
                        <td><b>Current HP</b>: ${final_stats["HP"]}</td>
                        </tr>
                        
                        <tr>
                        <td><b>Max ATK</b>: ${pokemon_obj["stat_atk"]}+${buffs_stats["ATK"]}+${points_by_stats["ATK"]}=${final_stats["ATK"]}</td>
                        <td><b>Current ATK</b>: ${final_stats["ATK"]}</td>
                        </tr>
                        
                        <tr>
                        <td><b>Max DEF</b>: ${pokemon_obj["stat_def"]}+${buffs_stats["DEF"]}+${points_by_stats["DEF"]}=${final_stats["DEF"]}</td>
                        <td><b>Current DEF</b>: ${final_stats["DEF"]}</td>
                        </tr>
                        
                        <tr>
                        <td><b>Max SPATK</b>: ${pokemon_obj["stat_sp_atk"]}+${buffs_stats["SPATK"]}+${points_by_stats["SPATK"]}=${final_stats["SPATK"]}</td>
                        <td><b>Current SPATK</b>: ${final_stats["SPATK"]}</td>
                        </tr>
                        
                        <tr>
                        <td><b>Max SPDEF</b>: ${pokemon_obj["stat_sp_def"]}+${buffs_stats["SPDEF"]}+${points_by_stats["SPDEF"]}=${final_stats["SPDEF"]}</td>
                        <td><b>Current SPDEF</b>: ${final_stats["SPDEF"]}</td>
                        </tr>
                        
                        <tr>
                        <td><b>Max SPEED</b>: ${pokemon_obj["stat_spd"]}+${buffs_stats["SPD"]}+${points_by_stats["SPD"]}=${final_stats["SPD"]}</td>
                        <td><b>Current SPEED</b>: ${final_stats["SPD"]}</td>
                        </tr>
                        </table>
                        `;
        ...
        move_list.map(move => {
            ...
            if (added_moves < 6) {
                ...
                normal_array.push(`
                                ...
                                <td>${move['AC'].toLowerCase() === "none"?"/":move['AC']}</td>
                                <td>${move['type']}</td>
                                <td>${final_roll}</td>
                                <td>${move['classe']}</td>
                                <td>${move['range']}</td>
...
function compute_points_automatically(pokemon_obj,available_points){
    ...
    return {"HP":point_in_hp,"ATK":point_in_atk,"DEF":point_in_def,"SPATK":point_in_spatk,"SPDEF":point_in_spdef,"SPD":point_in_spd}
}

function computeStatsBuff(pokemon_obj,card,rarity,final_buffed_stat,final_lowered_stat,bonus_point_nature_stat=2){
    let hp_auto_buff = 0;
    let atk_auto_buff = 0;
    let def_auto_buff = 0;
    ...
    return {"HP":hp_auto_buff,"ATK":atk_auto_buff,"DEF":def_auto_buff,"SPATK":spatk_auto_buff,"SPDEF":spdef_auto_buff,"SPD":spd_auto_buff}
}
function computeFinalStats(pokemon_obj,card,rarity,final_buffed_stat,final_lowered_stat,points_by_stats){
    let buffs = computeStatsBuff(pokemon_obj,card,rarity,final_buffed_stat,final_lowered_stat);
    let final_hp = Math.max(0,pokemon_obj["stat_hp"]+buffs["HP"]+points_by_stats["HP"])
    let final_atk = Math.max(0,pokemon_obj["stat_atk"]+buffs["ATK"]+points_by_stats["ATK"])
    let final_def = Math.max(0,pokemon_obj["stat_def"]+buffs["DEF"]+points_by_stats["DEF"])
    let final_spatk = Math.max(0,pokemon_obj["stat_sp_atk"]+buffs["SPATK"]+points_by_stats["SPATK"])
    let final_spdef = Math.max(0,pokemon_obj["stat_sp_def"]+buffs["SPDEF"]+points_by_stats["SPDEF"])
    let final_speed = Math.max(0,pokemon_obj["stat_spd"]+buffs["SPD"]+points_by_stats["SPD"])
    return {"HP":final_hp,"ATK":final_atk,"DEF":final_def,"SPATK":final_spatk,"SPDEF":final_spdef,"SPD":final_speed}
}
function PokemonGenerator() {
    ...
    if (pokemon_obj  && rarity !== "" && card !== "" && final_buffed_stat !== "" && final_lowered_stat !== ""){
        buffs_stats = computeStatsBuff(pokemon_obj,card,rarity,final_buffed_stat,final_lowered_stat);
    }
    let final_stats = {};
    if (rarity !== "" && card !== "" && final_buffed_stat !== "" && final_lowered_stat !== "" && (level > 0 && available_points === 0)) {
        final_stats = computeFinalStats(pokemon_obj,card,rarity,final_buffed_stat,final_lowered_stat,points_by_stats)
    }
        ...
        <div className="pokemon-generator-page">
            {
                ...
                <div className="pokemon-generator-sidebar">
                    ...
                    <Table responsive className="pokemon-generator-table" style={{borderColor:'white',borderWidth:'1px',borderStyle:'solid',minWidth:'90%',maxWidth:'90%'}}>
                        ...
                        <tr>
                            <th>HP</th>
                            <th>{(buffs_stats["HP"]>0?"+"+buffs_stats["HP"]:buffs_stats["HP"])}</th>
                        </tr>
                        <tr>
                            <th>ATK</th>
                            <th> {(buffs_stats["ATK"]>0?"+"+buffs_stats["ATK"]:buffs_stats["ATK"])}</th>
                        </tr>
                        <tr>
                            <th>DEF</th>
                            <th>{(buffs_stats["DEF"]>0?"+"+buffs_stats["DEF"]:buffs_stats["DEF"])}</th>
                        </tr>
                        <tr>
                            <th>SPATK</th>
                            <th>{(buffs_stats["SPATK"]>0?"+"+buffs_stats["SPATK"]:buffs_stats["SPATK"])}</th>
                        </tr>
                        <tr>
                            <th>SPDEF</th>
                            <th>{(buffs_stats["SPDEF"]>0?"+"+buffs_stats["SPDEF"]:buffs_stats["SPDEF"])}</th>
                        </tr>
                        <tr>
                            <th>SPD</th>
                            <th>{(buffs_stats["SPD"]>0?"+"+buffs_stats["SPD"]:buffs_stats["SPD"])}</th>
                        </tr>
                        </tbody>
                    </Table>
                    <br></br>
                    ...
                    <Table responsive className="pokemon-generator-table" style={{borderColor:'white',borderWidth:'1px',borderStyle:'solid',minWidth:'90%',maxWidth:'90%'}}>
                        ...
                        <tr>
                            <th>HP</th>
                            <th>{final_stats["HP"]}</th>
                        </tr>
                        <tr>
                            <th>ATK</th>
                            <th> {final_stats["ATK"]}</th>
                        </tr>
                        <tr>
                            <th>DEF</th>
                            <th>{final_stats["DEF"]}</th>
                        </tr>
                        <tr>
                            <th>SPATK</th>
                            <th>{final_stats["SPATK"]}</th>
                        </tr>
                        <tr>
                            <th>SPDEF</th>
                            <th>{final_stats["SPDEF"]}</th>
                        </tr>
                        <tr>
                            <th>SPD</th>
                            <th>{final_stats["SPD"]}</th>
                        </tr>
                        </tbody>
                    </Table>
                    <br></br>
                    ...
                    <Table responsive className="pokemon-generator-table" style={{borderColor:'white',borderWidth:'1px',borderStyle:'solid',minWidth:'90%',maxWidth:'90%'}}>
                        ...
                        <tr>
                            <th>Base Ability</th>
                            <th>{base_ability.map(abilitiy => {
                                return abilitiy +" \n "
                            })}</th>
                        </tr>
                        <tr>
                            <th>Advanced ability</th>
                            <th> {advanced_ability.map(abilitiy => {
                                return abilitiy +" \n "
                            })}</th>
                        </tr>
                        <tr>
            ...
            <div className="pokemon-generator-main">
            ...
            </Select>
                {
                    ...
                    <div>
                        ...
                        <Button disabled={level===-1} style={{marginLeft:'5px',marginTop:'5px',width:'5%',height:'30px'}} onClick={() => {
                            let points = compute_points_automatically(pokemon_obj,available_points)
                            dispatch(setPointsByStat({pokemon_points_by_stats:points}));
                        }}> Auto </Button>
                        <br></br>
                        <br></br>
                        ...
                        <div>
                            ...
                            <Button disabled={!ready_to_generate} style={{marginLeft:'5px',width:'15%',height:'50px'}} onClick={() => {
                                let final = exportSheet("markdown",moves,base_ability,advanced_ability,high_ability,final_stats,buffs_stats,points_by_stats,chosen_moves,egg_moves,tmhm_moves,locked_egg_moves,local_png_img,choosen_pokemon,card,gender,NATURE_MATRIX,final_buffed_stat,final_lowered_stat,level,pokemon_obj,rarity,notes,abilities);
                            ...
                            <Button disabled={!ready_to_generate} style={{marginLeft:'5px',alignContent:"center",width:'15%',height:'50px'}} onClick={() => {
                                exportSheet("googledoc",moves,base_ability,advanced_ability,high_ability,final_stats,buffs_stats,points_by_stats,chosen_moves,egg_moves,tmhm_moves,locked_egg_moves,local_png_img,choosen_pokemon,card,gender,NATURE_MATRIX,final_buffed_stat,final_lowered_stat,level,pokemon_obj,rarity,notes,abilities);
